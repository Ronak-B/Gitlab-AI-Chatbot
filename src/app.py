...
st.set_page_config(
    page_title="GitLab AI Chatbot",  # 🏷 Title shown in browser tab
    page_icon="🤖",                  # Can be emoji or image URL/path
...
if "promptk" not in st.session_state:
    st.session_state.promptk = ""

def on_prompt_submit():
    prompt = st.session_state.promptk
    if prompt:
        st.session_state.messages.append({"role": "user", "content": prompt})
        st.session_state.waiting = True

# Render chat history (always after new messages are appended)
...
# Only process assistant response if waiting is True and promptk is not empty
if st.session_state.waiting and st.session_state.promptk:
    with st.chat_message("assistant"):
        with st.spinner("Generating response..."):
            response, sources = st.session_state.chatbot.generate_response(st.session_state.promptk)
        if sources:
            gitlab_svg = (
            ...
            sources_html = (
                ...
                f'{gitlab_svg}<b>Sources:</b>&nbsp;{links}'
                f'</span>'
            )
            response += "<br>" + sources_html
